...
TEST_CASE("Direction check") {
    Snake snake(10);
    CHECK (snake.delta == Cell(0, -1));
    snake.left();
    CHECK (snake.delta == Cell(-1, 0));
    snake.right();
    CHECK (snake.delta == Cell(1, 0));
    snake.down();
    CHECK (snake.delta == Cell(1, 0));
...
TEST_CASE("Eating apple check"){
    Snake snake(10);
    Cell next = snake.body[0] + snake.delta;
...
TEST_CASE("Win check"
    ...
    for (int i = 1; i < snake.field.body.size() - 1; i++)
        for (int j = 1; j < snake.field.body[0].size() - 1; j++)
            if (snake.field.body[i][j] != Snake_id and Cell(i, j) != next){
